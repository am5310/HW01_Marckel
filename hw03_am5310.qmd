...
practcon <- dbConnect(duckdb::duckdb(dbdir = "./data/flights.duckdb", read_only = FALSE))

dbIsValid(practcon)
dbListTables(practcon)
dbListFields(practcon, "flights")

codechunk <- "```{sql, connection=practcon}\n\n```"
writeLines(codechunk)

result <- dbGetQuery(practcon, "SELECT * FROM flights LIMIT 5;")
